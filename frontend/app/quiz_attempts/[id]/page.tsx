...
export default function QuizAttemptsPage() {
  ...
  const scoreCounts = submittedAttempts
    .filter((attempt) => attempt.score !== null)
    .reduce<Record<string, number>>((counts, attempt) => {
      const score = Number(attempt.score).toFixed(0);
      counts[score] = (counts[score] || 0) + 1;
      return counts;
    }, {});

  const chartData = Object.entries(scoreCounts)
    .map(([score, count]) => ({
      score: Number(score),
      count,
    }))
  ...
  const maxScoreCount = Math.max(...chartData.map((item) => item.count), 1);
  ...
  useEffect(() => {
    async function fetchAttempts() {
      try {
        ...
        const response = await fetch(apiUrl(`/attempts/?quiz=${params.id}`), {
          headers: {
        ...
        {!isLoading && !error && chartData.length > 0 && (
          <div className="mt-8 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-8 overflow-x-auto">
              <div className="flex h-72 min-w-full items-end gap-4 border-b border-l border-slate-200 px-4 pb-4 pt-6">
              {chartData.map((item) => (
                <div
                  className="flex h-full min-w-20 flex-1 flex-col items-center justify-end gap-2"
                  key={item.score}
                >
                  ...
                  <div className="flex h-52 w-full items-end justify-center">
                    <div
                      className="w-14 rounded-t-md bg-teal-700 transition"
                      style={{
                        height: `${Math.max(
                          (item.count / maxScoreCount) * 100,
                          10,
                        )}%`,
                      }}
                    />
